...
```
 > Enemies will have a FOV (Field Of View), Each enemy type(race) will have a diffrent one. Factors that will effect base FOV:
   ...
   > How dark or light an area is (If there are tourches in a room or a player is holding one then the player will be easier to spot, but so will the enemies).
   > If anything is obscuring their view or if something is obscuring the player character (i.e a Wall). 
